function z = ppca_mm_ica_no_orth( X, R, q )
%=========================================================================
...
%
%   * In this version, w_i^T w_j is not required to be 0 for i not equal to j
%=========================================================================
%
%=========================================================================
%   OUTPUTS:
...
%               check input args
    if ( nargin ~= 3 )
        disp('Usage: z = ppca_mm_ica_no_orth(X, R, q)');
        z = [];
        return;
    end
%=========================================================================
...
%=========================================================================
%               next call projected_mog_ica without orthogonality
%               restriction
    % initialize W
    W = [];
    for k = 1:q
        ...
        % non-Gaussian densities)
        if ( k == 1 )
            pmog{k} = projected_mog_ica( ppca.marginal_source_mean, 5, [], 1e-5 );
        else
            winit = randn( size( ppca.marginal_source_mean, 1 ), 1 );
            winit = winit - W*inv(W'*W)*W'*winit;
            winit = winit/norm(winit);
            pmog{k} = projected_mog_ica( ppca.marginal_source_mean, 5, [], 1e-5, winit );
        end
        W = [W, pmog{k}.w];
        % The modified mixing matrices are given by ppca.A{k}*W
        z.pmog{k} = pmog{k};
...
%               (q by n matrix of final source estimates)
    ...
    if ( R == 1 )
        z.final_mixing_matrix = z.ppca.A{1}*inv(W');
    end
%=========================================================================
